...
export default function AdminPage() {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      const data = await fetchJson(getApiUrl("/api/admin/requests"), {
        headers: {
  ...
  const handleStatusUpdate = async (requestId, status) => {
    ...
    try {
      const data = await fetchJson(getApiUrl(`/api/admin/requests/${requestId}/status`), {
        method: "PATCH",
        headers: {
  ...
  const handleDoneTrip = async (requestId) => {
    setError("");
    setIsUpdatingId(requestId);

    try {
      await fetchJson(getApiUrl(`/api/admin/requests/${requestId}`), {
        method: "DELETE",
        headers: {
          "x-admin-password": password,
        },
        ...
        {!isLoggedIn ? (
          ...
          <div className="admin-requests">
            <div className="admin-toolbar">
              <div>
                <p className="admin-count">{pendingRequests.length} pending request(s)</p>
                <p className="admin-count">{confirmedRequests.length} confirmed trip(s)</p>
              </div>
              <button
                type="button"
            ...
            <section className="admin-section">
              <div className="admin-section-header">
                ...
                <p>Review each request and confirm or reject it.</p>
              </div>
                          ...
                          <td>
                            <div className="admin-actions">
                              <button
                                ...
                                disabled={isUpdatingId === request._id}
            ...
            <section className="admin-section">
              <div className="admin-section-header">
                ...
                <p>Accepted bookings are arranged by trip date and boarding time.</p>
              </div>
                  ...
                  {confirmedRequests.map((request) => (
                    <article key={request._id} className="admin-schedule-card">
                      ...
                      <button
                        ...
                        disabled={isUpdatingId === request._id}
                        onClick={() => handleDoneTrip(request._id)}
                      >
                        Done Trip
                      </button>
                    </article>
